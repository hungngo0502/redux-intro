...
const CartProductOne = (props) => {
  ...
  const productCart1 = useSelector((state) => state.cart.products[0]);
  const product = { price: productCart1.price, qty: productCart1.qty };
...
const CartProductTwo = (props) => {
  ...
  // Change the price of products in `service/cart/slice.js` to see the effect
  const productCart2 = useSelector((state) => state.cart.products[1]);
  const product = { price: productCart2.price, qty: productCart2.qty };
  return (
    <WrapperBox>
      <Typography
        p="0.5rem"
